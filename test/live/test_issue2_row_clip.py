#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""test_issue2_row_clip.py — live regression for issue #2 (~2 minutes).

Issue #2 is a runtime VRAM-row alias: a 12px glyph write crosses a 13-tile
row and overwrites the lower half of the next row's first glyph.  The static
``glyph_row_clip`` gate pins the hook bytes; this test proves the three known
surfaces render complete glyphs in the real engine:

    * issue image 1: Extra -> Profile -> character list
    * issue image 2: Extra -> Profile -> unit list
    * issue image 3: Continue -> save slot 3 -> MS Development -> System Tree
...
of the oracle while making an unfixed v1.1 build fail decisively.

    .venv/bin/python test/live/test_issue2_row_clip.py <rom.nds> [options]
...
# DeSmuME captures, which lets the oracle be calibrated independently of the
# 19px melonDS menu bar.
...
def profile_metrics(frame, kind: str) -> dict[str, float | int | bool]:
    ...
    # row boundary, which carries unrelated one-pixel residue in the bad ROM.
...
def capture_profile(
    ...
    # issue images 1/2, rather than merely preserving the first cached frame.
...
def main() -> int:
    ...
    out = emu.workdir / "issue2_row_clip"
