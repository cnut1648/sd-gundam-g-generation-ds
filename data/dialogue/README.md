...
```jsonc
{
 ...
 "edits": [                  // ascending, non-overlapping byte-range replacements
  {
   "jp_offset": "0x111e",    // offset in the ORIGINAL (Japanese) file
   "jp_len": 9,              // bytes replaced there
   "kind": "dialogue",       // "dialogue" = one clean text block; "script" = raw range
   "speaker": "夏亚",         // (dialogue only, optional) speaker of the line
   "jp_text": "……どうだ？",   // (dialogue only) decoded original text — reference
   "zh_text": "……情况如何？", // (dialogue only) decoded translation — reference
   "jp_hex": "…",            // original bytes — reference
...
* `zh_hex` is authoritative: the build splices exactly these bytes. The text
  fields are decoded FROM the byte fields for human reading/editing and are
  never re-encoded by the build (see `docs/` for the encoding convention if
  you want to author new lines).
